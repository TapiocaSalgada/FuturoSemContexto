...
export default function AnimeCard({
  ...
  return (
    <Link
      prefetch={true}
      href={href}
      className={`group block shrink-0 snap-start focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-strong)] focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--surface-0)] rounded-2xl ${className}`}
    >
      <article className="relative aspect-[2/3] overflow-hidden rounded-2xl border border-white/12 bg-[var(--surface-1)] transition-all duration-300 group-hover:-translate-y-1 group-hover:border-white/28 group-hover:shadow-[0_22px_40px_rgba(0,0,0,0.56)] group-active:scale-[0.99]">
        {isImageLoading ? <div className="absolute inset-0 kdr-skeleton z-[1]" /> : null}
        {hasImage ? (
          <img
            src={currentImage}
            alt={title}
            ...
            loading="lazy"
            decoding="async"
            sizes="(max-width: 640px) 42vw, (max-width: 1024px) 28vw, 16vw"
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/32 to-transparent" />
        ...
        {!hideTitle ? (
          <div className="absolute bottom-0 left-0 right-0 z-10 p-3">
            <p className="line-clamp-2 text-[12px] leading-[1.25] font-extrabold tracking-[0.01em] text-white drop-shadow">
              {title}
            </p>
          </div>
        ) : null}
        ...
        <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center opacity-0 transition-opacity duration-250 group-hover:opacity-100">
          <span className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/80 bg-white text-black shadow-lg">

...
export default function HistoryPage() {
  ...
  if (status === "loading" || loading) {
    return (
      <AppLayout>
        <div className="kdr-page-shell max-w-6xl kdr-page-stack animate-fadeIn">
      ...
      <div className="kdr-page-shell max-w-6xl kdr-page-stack">
        <div className="kdr-page-hero">
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <p className="kdr-page-eyebrow">Retomar maratona</p>
              ...
              <p className="kdr-page-lead">Todos os episódios assistidos. Você pode remover itens individuais ou limpar tudo.</p>
            </div>
            <button
              onClick={clearAll}
              disabled={clearing || uniqueHistory.length === 0}
              className="w-full sm:w-auto px-4 py-2 rounded-xl text-xs font-black uppercase tracking-wide bg-purple-600/20 border border-purple-500/30 text-purple-300 hover:bg-purple-600 hover:text-white disabled:opacity-50"
            >
        ...
        {uniqueHistory.length === 0 ? (
          <div className="text-center py-20 text-zinc-500">
            ...
            <Link prefetch={true} href="/" className="mt-6 inline-flex items-center justify-center bg-white hover:bg-zinc-100 text-black font-black px-6 py-2.5 rounded-full text-sm transition shadow-[0_0_20px_rgba(255,255,255,0.2)]">
              Explorar catálogo
            </Link>
          </div>
        ) : (
          <div className="space-y-2.5">
            {uniqueHistory.map((historyItem) => {
              const anime = historyItem.episode?.anime;
              const ep = historyItem.episode;
              if (!anime || !ep) return null;
              return (
                <div key={historyItem.id} className="flex items-center gap-4 p-4 glass-card border border-white/10 rounded-2xl">
                  <Link prefetch={true} href={`/watch/${ep.id}`} className="flex items-center gap-4 min-w-0 flex-1 group">
                    <div className="relative w-24 h-14 sm:w-28 sm:h-16 rounded-xl overflow-hidden shrink-0 bg-zinc-800">
                      <Image
                        src={ep.thumbnailUrl || anime.bannerImage || anime.coverImage || "/logo.png"}
                        alt={anime.title}
                        fill
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <p className="text-[10px] sm:text-[11px] text-zinc-500 font-medium uppercase tracking-wider">{new Date(historyItem.updatedAt).toLocaleDateString("pt-BR")}</p>
                    <button

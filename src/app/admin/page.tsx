...
function ImageUpload({ onUpload, label }: { onUpload: (url: string) => void; label: string }) {
  ...
  return (
    <label className="flex items-center gap-2 cursor-pointer text-pink-500 hover:text-pink-400 text-xs font-bold transition">
      <UploadCloud size={14} />
...
export default function AdminDashboard() {
  ...
  const loadUsers = useCallback(() => fetch("/api/admin/users").then(r => r.json()).then(setUsers), []);
  ...
  const handleAnimeSubmit = async (e: React.FormEvent) => {
    ...
    if (res.ok) { showMsg(editingAnime ? "Anime editado!" : "Anime salvo!"); setAnimeForm({ title: "", description: "", coverImage: "", bannerImage: "", status: "ongoing" }); setEditingAnime(null); loadAnimes(); }
  ...
  const handleTimeout = async (userId: string) => {
    const until = timeoutForm[userId];
    await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: userId, timeoutUntil: until || null }) });
    showMsg("Punição aplicada!"); loadUsers();
  };

  const handleEditUser = async (userId: string) => {
    const form = editUserForm[userId];
    if (!form) return;
    await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: userId, ...form }) });
    showMsg("Usuário editado!"); loadUsers(); setExpandedUser(null);
  };
  ...
  const handleSuggestionStatus = async (id: string, status: string) => {
    await fetch("/api/suggestions", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id, status }) });
    loadSuggestions();
  };
  ...
  return (
    <AppLayout>
      <div className="p-6 lg:p-10 pb-24 max-w-7xl mx-auto space-y-6">
        ...
        {msg && (
          <div className={`p-3 rounded-xl text-sm font-bold border ${msgType === "ok" ? "bg-green-500/10 text-green-400 border-green-500/30" : "bg-red-500/10 text-red-400 border-red-500/30"}`}>
            {msg}
          </div>
        )}
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-4">
                <h2 className="font-bold text-lg">{editingAnime ? "Editar Anime" : "Adicionar Anime"}</h2>
                <form onSubmit={handleAnimeSubmit} className="space-y-3">
                  <div><label className={labelClass}>Título</label><input required className={inputClass} value={animeForm.title} onChange={e => setAnimeForm({ ...animeForm, title: e.target.value })} placeholder="Ex: Naruto Shippuden" /></div>
                  <div><label className={labelClass}>Sinopse</label><textarea className={inputClass + " min-h-[80px] resize-none"} value={animeForm.description} onChange={e => setAnimeForm({ ...animeForm, description: e.target.value })} placeholder="Descrição..." /></div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className={labelClass}>Capa</label>
                      <input className={inputClass} value={animeForm.coverImage} onChange={e => setAnimeForm({ ...animeForm, coverImage: e.target.value })} placeholder="URL ou upload" />
                      <div className="mt-1"><ImageUpload label="Fazer upload" onUpload={url => setAnimeForm({ ...animeForm, coverImage: url })} /></div>
                    </div>
                    <div>
                      <label className={labelClass}>Banner</label>
                      <input className={inputClass} value={animeForm.bannerImage} onChange={e => setAnimeForm({ ...animeForm, bannerImage: e.target.value })} placeholder="URL ou upload" />
                      <div className="mt-1"><ImageUpload label="Fazer upload" onUpload={url => setAnimeForm({ ...animeForm, bannerImage: url })} /></div>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                  ...
                  <div className="flex gap-2">
                    <button type="submit" className="flex-1 bg-pink-600 hover:bg-pink-500 text-white font-bold py-2.5 rounded-lg text-sm transition flex items-center justify-center gap-2">
                      <Plus size={16} /> {editingAnime ? "Atualizar" : "Adicionar"}
                    </button>
                    {editingAnime && (
                      <button type="button" onClick={() => { setEditingAnime(null); setAnimeForm({ title: "", description: "", coverImage: "", bannerImage: "", status: "ongoing" }); }}
              ...
              <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-4">
                ...
                <form onSubmit={handleEpSubmit} className="space-y-3">
                  ...
                  <div>
                    ...
                    <input required className={inputClass} value={epForm.videoUrl} onChange={e => setEpForm({ ...epForm, videoUrl: e.target.value })} placeholder="/videos/ep1.mp4 ou https://..." />
                    <div className="mt-1"><ImageUpload label="Upload de vídeo" onUpload={url => setEpForm({ ...epForm, videoUrl: url })} /></div>
                    {localFiles.length > 0 && (
                      <div className="mt-2 flex flex-wrap gap-1">
                        {localFiles.map(f => (
                          <button key={f} type="button" onClick={() => setEpForm({ ...epForm, videoUrl: `/videos/${f}` })}
                            className="text-xs bg-zinc-800 hover:bg-pink-600 border border-zinc-700 px-2 py-1 rounded transition truncate max-w-[120px]">
                            {f}
                          </button>
                        ))}
                      </div>
            ...
            <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={loadAnimes} className="text-zinc-500 hover:text-white transition"><RefreshCw size={16} /></button>
              </div>
              <div className="space-y-2">
                {animes.map((a) => (
                  <div key={a.id} className="flex items-center gap-3 p-3 bg-zinc-800/50 rounded-xl hover:bg-zinc-800 transition group">
                    {a.coverImage && <img src={a.coverImage} alt={a.title} className="w-10 h-14 object-cover rounded-lg shrink-0" />}
                    {!a.coverImage && <div className="w-10 h-14 bg-zinc-700 rounded-lg shrink-0 flex items-center justify-center"><Film size={16} className="text-zinc-500" /></div>}
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition">
                      ...
                      <button onClick={() => { setEditingAnime(a.id); setAnimeForm({ title: a.title, description: "", coverImage: a.coverImage || "", bannerImage: "", status: a.status }); setTab("animes"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
        ...
        {tab === "users" && (
          <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={loadUsers} className="text-zinc-500 hover:text-white transition"><RefreshCw size={16} /></button>
            </div>
            <div className="space-y-3">
              {users.map(u => {
                const timedOut = u.isTimedOut && new Date(u.isTimedOut) > new Date();
                const expanded = expandedUser === u.id;
                if (!editUserForm[u.id]) editUserForm[u.id] = { name: u.name, email: u.email, role: u.role };
                return (
                    ...
                    <div className="flex items-center gap-3 p-3 cursor-pointer hover:bg-zinc-800 transition" onClick={() => setExpandedUser(expanded ? null : u.id)}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          {u.role === "admin" && <span className="text-xs bg-pink-500/20 text-pink-400 px-2 py-0.5 rounded-full font-bold">Admin</span>}
                    ...
                    {expanded && (
                      <div className="border-t border-zinc-700 p-4 space-y-4 bg-zinc-900/50">
                        ...
                        <div className="border-t border-zinc-700 pt-4">
                          ...
                          <div className="flex items-center gap-2">
                            <input type="datetime-local" className={inputClass + " flex-1"}
                            ...
                            {timedOut && (
                              <button onClick={() => { setTimeoutForm({ ...timeoutForm, [u.id]: "" }); handleTimeout(u.id); }}
                                className="px-4 py-2.5 bg-zinc-700 hover:bg-zinc-600 text-white text-sm font-bold rounded-lg transition whitespace-nowrap">
                                Remover
                              </button>
                            )}
                          </div>
                          {timedOut && <p className="text-xs text-red-400 mt-1">Silenciado até: {new Date(u.isTimedOut!).toLocaleString("pt-BR")}</p>}
                        </div>
        ...
        {tab === "suggestions" && (
          <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-lg">Sugestões de Animes ({suggestions.length})</h2>
              <button onClick={loadSuggestions} className="text-zinc-500 hover:text-white transition"><RefreshCw size={16} /></button>
            </div>
            {suggestions.length === 0 && <p className="text-zinc-500 text-sm">Nenhuma sugestão ainda.</p>}
            <div className="space-y-3">
              {suggestions.map(s => (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <img src={s.user.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(s.user.name)}&background=333&color=fff`} className="w-5 h-5 rounded-full" alt={s.user.name} />
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full font-bold shrink-0 ${s.status === "pending" ? "bg-yellow-500/20 text-yellow-400" : s.status === "accepted" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}>
          ...
          <div className="space-y-6">
            <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6">
              ...
              <form onSubmit={handleAnnSubmit} className="space-y-3">
                ...
                <div><label className={labelClass}>Mensagem</label><textarea required className={inputClass + " min-h-[100px] resize-none"} value={annForm.content} onChange={e => setAnnForm({ ...annForm, content: e.target.value })} placeholder="Conteúdo do anúncio..." /></div>
            ...
            <section className="bg-zinc-900/40 border border-zinc-800 rounded-2xl p-6 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <button onClick={loadAnnouncements} className="text-zinc-500 hover:text-white transition"><RefreshCw size={16} /></button>
              </div>
              {announcements.length === 0 && <p className="text-zinc-500 text-sm">Nenhum anúncio ainda.</p>}
              {announcements.map(a => (
                <div key={a.id} className="flex items-start justify-between gap-4 bg-zinc-800/50 rounded-xl p-4 border border-zinc-700/50">
                  <div>
                    <p className="font-bold text-white">{a.title}</p>
                    <p className="text-sm text-zinc-400 mt-1">{a.content}</p>
                    <p className="text-xs text-zinc-600 mt-2">{new Date(a.createdAt).toLocaleString("pt-BR")}</p>
                  </div>
                  <button onClick={() => handleDeleteAnn(a.id)} className="p-2 hover:bg-red-500/20 text-zinc-500 hover:text-red-400 rounded-lg transition shrink-0"><Trash2 size={16} /></button>
                </div>
              ))}
            </section>
          </div>

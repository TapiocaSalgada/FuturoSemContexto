...
export default async function HomePage() {
  ...
  const trendingEpisodes = trendingEpisodeIds.length > 0
    ? await prisma.episode.findMany({
        where: { id: { in: trendingEpisodeIds }, anime: { visibility: "public" } },
  ...
  return (
    <AppLayout>
      <div className="pb-24">
        {/* Hero Banner */}
        {featured ? (
          <section className="relative w-full h-[52vh] lg:h-[62vh] flex flex-col justify-end p-8 lg:p-14 overflow-hidden">
            <div className="absolute inset-0 z-0">
              ...
              {/* Cinematic vignette */}
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,transparent_40%,rgba(6,6,6,0.8)_100%)]" />
            </div>
            <div className="relative z-10 max-w-2xl space-y-4 animate-fadeInUp">
              <div className="flex items-center gap-2">
                <Heart size={12} className="text-pink-500 fill-pink-500" />
                <span className="text-pink-500 font-bold tracking-widest text-xs uppercase">Em Destaque</span>
              </div>
                ...
                {trendingAnimes.map((ep, i) => (
                  <Link key={ep.animeId} href={`/anime/${ep.animeId}`}
                    ...
                    <div className="aspect-[2/3] rounded-xl overflow-hidden relative border border-zinc-800 group-hover:border-pink-500 transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(255,0,127,0.2)]">
                      <img src={ep.anime?.coverImage || ""} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" alt={ep.anime?.title || ""} />
              ...
              <div className="flex gap-4 overflow-x-auto pb-3 scrollbar-hide snap-x">
                {recentAnimes.map(anime => (
                  <Link key={anime.id} href={`/anime/${anime.id}`}
                    className="w-[130px] lg:w-[160px] shrink-0 snap-start group">
                    <div className="aspect-[2/3] rounded-xl overflow-hidden relative border border-zinc-800 group-hover:border-pink-500 transition-all duration-300 group-hover:shadow-[0_0_20px_rgba(255,0,127,0.2)]">
                      <img src={anime.coverImage || ""} className="w-full h-full object-cover group-hover:scale-110 transition duration-500" alt={anime.title} />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition duration-300 flex items-end p-3">
                        <div>
